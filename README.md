...
Задание содержит 25 задач. Выполнены задачи 1--24 (включая бонусные). Остается задача 25.
...
Открываем `portfolio_analysis.ipynb` и выполняем ячейки. Все данные уже лежат в `data/` -- ничего скачивать не нужно. Для Excel: сводный файл `data/export/data_export.xlsx` содержит все основные данные на 39 листах.
...
| 23*** | Monte Carlo frontier | Выполнено | `data/processed/step11_mc_*.parquet` |
| 24**** | Наиболее рискованный портфель | Выполнено | `data/processed/step12_*.parquet`, `data/processed/step12_*.pkl` |
...
### Monte Carlo frontier (задача 23)

MC-симуляция: 500 000 случайных портфелей для каждого из 3 режимов ограничений. Генерация весов: Dirichlet(1,...,1) для long-only, нормальное распределение с нормализацией для unrestricted, rejection sampling для short <= 25%. MC frontier извлекается как upper envelope (200 бинов по return, min std в каждом).

| Режим | MC GMVP Std | Anal. GMVP Std | Gap | MC Tangency Sharpe | Anal. Tangency Sharpe | Gap |
|-------|-------------|----------------|-----|--------------------|-----------------------|-----|
| Long only | 20.1% | 17.4% | 15.4% | 0.226 | 1.468 | 84.6% |
| Unrestricted | 16.9% | 13.6% | 24.0% | 3.619 | 4.048 | 10.6% |
| Short <= 25% | 17.0% | 13.6% | 24.9% | 2.085 | 4.077 | 48.9% |

Все MC frontiers лежат внутри (ниже-правее) аналитических -- подтверждает оптимальность решений Марковица. MC tangency Sharpe положительный для всех режимов, хотя аналитический tangency на long-only границе тоже положителен (но значительно выше).

Сходимость: зазор GMVP std убывает с ростом N, но при K=30 активах Dirichlet(1,...,1) концентрирует вес вблизи equal-weight (concentration of measure), что замедляет сходимость. Для unrestricted режима (нормальное распределение) сходимость лучше -- Sharpe gap всего 10.6%.

### Наиболее рискованный портфель (задача 24)

Обратная задача к Марковицу: max w'Sigma*w вместо min. Доказано, что при единственном ограничении 1'w = 1 максимум не существует (sup = +infinity) -- единственная критическая точка Лагранжиана это GMVP (минимум). Риск можно наращивать неограниченно через long-short позиции.

Направление максимального роста риска -- первый собственный вектор проецированной ковариационной матрицы. Eigenvector portfolio w_eig = v1 / (1'v1) -- портфель с максимальной экспозицией к рыночному фактору (все компоненты одного знака, что подтверждает интерпретацию первого собственного вектора как рыночного фактора).

Anti-efficient frontier (граница наиболее рискованных портфелей) построена при ограничениях на леверидж ||w||1 <= L для L = 1, 1.5, 2, 3, 5, 7, 10, 15, 20. Задача невыпуклая -- решается multi-start SLSQP. Нижняя ветвь MVS (inefficient frontier) рассчитана аналитически по формуле Мертона как зеркальное отражение efficient frontier.

---

## Оставшаяся задача (25)
...
**data/processed/** -- обработанные данные (задачи 1--24):
...
| `step11_mc_comparison.parquet` | 23 | Сравнение MC vs аналитических портфелей |
| `step11_mc_convergence.parquet` | 23 | Сходимость MC (N от 1k до 500k) |
| `step11_mc_key_portfolios.pkl` | 23 | MC GMVP, tangency, max_return для 3 режимов |
| `step12_eigen_analysis.pkl` | 24 | Спектральное разложение Σ, Merton scalars |
| `step12_max_risk_direction.parquet` | 24 | Направление d* максимального роста риска |
| `step12_max_risk_trajectory.parquet` | 24 | Параметрическая траектория из GMVP (100 точек) |
| `step12_leverage_frontier.parquet` | 24 | Max σ² при ограничениях на леверидж (10 уровней) |
| `step12_anti_ef.parquet` | 24 | Anti-efficient frontier (L=2,3,5,10, 250 точек) |
| `step12_anti_ef_weights.pkl` | 24 | Веса anti-EF портфелей |
| `step12_inefficient_frontier.parquet` | 24 | Нижняя ветвь MVS (200 точек) |
| `step12_comparison.parquet` | 24 | Сравнение всех границ (42 метрики) |
| `step12_key_portfolios.pkl` | 24 | Ключевые портфели: GMVP, eigenvector, leverage |

**data/export/**:

| Файл | Описание |
|------|----------|
| `data_export.xlsx` | Все данные в Excel (39 листов, описание на листе spravka) |
...
21 файл с тестами (pytest), проверяют корректность данных и расчётов по всем задачам:
...
| `test_step11_phase_a.py` | 23 | ~47 |
| `test_step11_phase_c.py` | 23 | ~20 |
| `test_step12_phase_a.py` | 24 | ~44 |
| `test_step12_phase_b.py` | 24 | ~40 |
...
| mc_comparison | MC vs аналитические портфели (3 режима) |
| mc_convergence | Сходимость MC GMVP/Sharpe по N |
| mc_key_portfolios | MC GMVP, tangency для 3 режимов |
| max_risk_eigen | Спектральное разложение Σ, eigenvector portfolio |
| max_risk_trajectory | Параметрическая траектория max-risk |
| max_risk_leverage | Max σ² vs уровень левериджа |
| max_risk_anti_ef | Anti-efficient frontier (4 уровня L) |
